...
function DrawObjects({
      ...
      $(function () {
        ...
        for(var i = 0; i < coordinates.length; i ++){
          ...
          (function(i){
            ...
            handle.style.borderRadius = '50px';
            // handle.style.border = '0px';


            // remove point if exist
            $(`#${objectName}.point_${i}`).remove()
            $("#image_panel").append(handle);
            ...
            $(handle).draggable({
              drag: function (event) {
                setTimeout(function () { // jQuery apparently calls this *before* setting position, so defer
                    ...
                    let point =  returnPossiblePoint(left,top);
                    handle.style.left = base.left + point.x + "px";
            handle.style.top = base.top + point.y + "px";
                  // setObjectName({
                  //   ...objectName, 
  ...
  const returnPossiblePoint = (x,y) => {
    x=(x>0)?x:0;
    y=(y>0)?y:0;
    let base = $("#image_panel");
    x = (x>base.width()-5)?base.width()-5:x;
    y = (y>base.height()-5)?base.height()-5:y;
  ...
  return (
    <>
      {coordinates&&objectName&&<div
        ...
        // }}
        // style'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''={{
          top: (coordinates[0]?.y + coordinates[1]?.y)/2 -30 + "px",
          left:(coordinates[0]?.x + coordinates[1]?.x)/2-100 + "px",
          // width: coordinates[1]?.x - coordinates[0]?.x + "px",
      ...
      >
        <div style={{display:'flex',zIndex:5000}}>
          <div className="object-box-key text-truncate"style={{backgroundColor: `rgb(${c?.color?.join(",")})`,padding:'0px 20px'}}>
            {c?.key?.slice(0,c?.key?.indexOf("0") > -1 ? c?.key?.indexOf("0") : c?.key?.length)}
          </div>
          <div className="object-button text-primary cursor-pointer" style={{zIndex: isDeletingObject ? 0 : 91,}} onClick={() => (!isDeletingObject ? distoryObject(c) : "")}>
